...
def gen_max_hamming_set(N):
    ...
    full_permutation_set = np.zeros((NUM_PERMUTATIONS, 4), dtype=np.uint8)
    for i, el in enumerate(itertools.permutations(range(4), 4)):
        full_permutation_set[i] = el
    max_hamming_sets = np.zeros((N, 4), dtype=np.uint8)
    ...
    for i in range(N):
        ...
        full_permutation_set[NUM_PERMUTATIONS -
                             (i + 1)] = np.zeros((1, 4), dtype=np.uint8)
...
if __name__ == "__main__":
    if len(sys.argv) == 2:
        ...
        print("Number of permutations not specified, using 100")
        main()
